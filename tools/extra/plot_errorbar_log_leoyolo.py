...
def plot_chart(chart_type, path_to_png, path_to_log_list):
    ...
    for path_to_log in path_to_log_list:
        ...
        if count==0:
            ...
            ally = np.stack((ally, data[1]))
        else:
            data[1] = [data[1]]
            ally = np.append(ally, data[1], axis=0)
        count = count + 1

    y = np.mean(ally, axis=0)
    miny = y - np.amin(ally, axis=0)
    maxy = np.amax(ally, axis=0) - y

    line,caps,bars=plt.errorbar(x, y, ecolor='r', yerr=[miny, maxy], capsize=5, errorevery=3, elinewidth=0.5)
    legend_loc = get_legend_loc(chart_type)
    plt.legend(loc = legend_loc, ncol = 1)
    plt.xlabel(x_axis_field)
    plt.ylabel(y_axis_field)
    plt.savefig(path_to_png, bbox_inches='tight')
    plt.show()
